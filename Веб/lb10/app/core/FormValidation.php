...
class FormValidation
{
    ...
    public function isFio($data) {
        // Pattern checks for 2-5 words with letters only (Latin and Cyrillic)
        $pattern = "/^[A-Za-zА-Яа-яЁё]+(\s+[A-Za-zА-Яа-яЁё]+){1,4}$/u";
        if(!preg_match($pattern, $data)){
            return "ФИО должно содержать от 2 до 5 слов, разделенных пробелами";
        }
        return null;
    }
