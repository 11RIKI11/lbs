...
<section class="page-box" id="guest-book-box">
        ...
        <form method="POST" action="/guestbook" name="guest-book" novalidate>
                <div id="name-input-box">
                        ...
                        <input type="text" id="name" name="inputName" class="inputUncheck" required
                ...
                <input type="email" id="emailInput" name="email" class="inputUncheck" required
                        value="<?= htmlspecialchars($formData['email'] ?? '') ?>">
                <?= isset($errorsTags['email']) ? ($errorsTags['email'][0] ?? '<br><br>') : '<br><br>'?>

                <label for="messageInput">Текст отзыва</label><br>
                <textarea id="messageInput" name="messageInput" rows="3" class="inputUncheck" required><?= htmlspecialchars($formData['inputMessage'] ?? '') ?></textarea>
                <?= isset($errorsTags['messageInput']) ? ($errorsTags['messageInput'][0] ?? '<br><br>') : '<br><br>'?>

                <button type="submit">Отправить</button>
                <button type="reset">Очистить</button>
                <button id="upload-button" type="button" onclick="window.location.href='/guestbook/upload'">Загрузить</button>
        </form>
</section>
